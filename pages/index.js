...
export default function Home() {
  ...
  const toolUrls = {
    ...
    homepage: "https://ainspire.co.kr", // 👈 새로운 홈페이지 주소 추가
  };
  ...
  useEffect(() => {
    ...
    // 2. 메인 콘텐츠 등장 타이머 (타이틀이 완전히 사라진 후 0.5초 뒤 등장)
  ...
  const mainContentVariants = {
    ...
    visible: {
      opacity: 1,
      transition: {
        staggerChildren: 0.1, // 카드 등장 간격 조정
      },
    },
  };
  ...
  // 4개 도구와 홈페이지 박스를 배열로 정의
  const toolBoxes = [
    // 🚨 1. ainspire 홈페이지 박스 (노란색으로 강조)
    {
      title: "ainspire 홈페이지",
      desc: "저희 회사의 최신 정보와 비전을 확인하실 수 있습니다.",
    ...
    {
      title: "스토리보드",
      desc: "아이디어를 시각화하고 스토리를 구성하는 데 도움을 줍니다.",
    ...
    {
      title: "이미지 툴",
      desc: "간단한 프롬프트만으로 놀라운 고품질 이미지를 생성합니다.",
    ...
    {
      title: "레퍼런스 수집기",
      desc: "영감을 줄 레퍼런스를 효율적으로 찾아 관리합니다.",
    ...
    {
      title: "프롬프트 생성기",
      desc: "AI에게 최적의 지시를 내릴 수 있는 프롬프트를 자동으로 만듭니다.",
  ...
  return (
    ...
    >
      ...
      {showMainContent && (
        <motion.main
          className="text-center w-full max-w-7xl bg-black bg-opacity-70 rounded-lg p-8 shadow-2xl relative z-0" // max-w-7xl로 확장하여 3개 이상도 충분히 담도록 함
        ...
        >
          ...
          {/* 🚨🚨 유연한 레이아웃: grid-cols-autofit 클래스 사용 */}
            ...
            {toolBoxes.map((box, index) => (
              ...
              >
                ...
                <div className="flex justify-between items-center mb-3">
                  <h2 className={`text-3xl font-bold ${box.color} font-pretendard`}>
                    {box.title}
                  </h2>
                  <span className={`text-sm font-semibold px-2 py-1 rounded-full border border-current ${box.tagColor} font-pretendard`}>
                    {box.tag}
                  </span>
                </div>
                
                {/* 설명 */}
                <p className="text-gray-400 mb-6 font-pretendard">
                  {box.desc}
                </p>
                
                {/* 링크 */}
                <span className={`${box.tagColor} font-semibold text-lg font-pretendard`}>
